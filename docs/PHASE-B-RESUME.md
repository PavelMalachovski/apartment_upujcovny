...
**Shipped on branch `phaseB-plan4c-exterior-layout` (2026-08-19, unmerged at the time of writing): serenity `exposure` **0.31**, kings-court **0.52**, horkyone-10 **0.42**; bloom unchanged at threshold 1.8 / strength 0.1; `?v=135`; serenity gains a `sky` key and two `photoSpots` gain a `pitch`.** Baselines: serenity **14.34**, kings-court **17.58–17.62**, both all-spot legacy and both paired same-session against `b78ebd3`.
...
| **Nine serenity spots and all thirteen kings-court spots are still captured at `pitch: 0`**, and every one was pose-verified under that constraint | On the two spots where the tilt was supplied it was worth **−12.51** and **−2.22** of ΔE, so this is not cosmetic. But sweeping it is a second pass over every camera in both flats — plan 4b's work redone — and it needs its own before/after | **4d** |
