...
| `baketime7.mjs` | `window.__bakeMs`, both sides, all three apartments, appended as a new batch each run. Task 7 quotes **no** bake-time claim from the loaded-machine batches and only a guarded one from the idle batches — this file is the evidence for that distinction |
