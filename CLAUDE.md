...
2026-08-22, plan 5 task 2, read off the current source (not measured, and not
...
built scene. `builder.js` enforces no cap — `builder.js:1599` adds one live
`PointLight` per `dyn` entry. **The rule is not being softened to make the
