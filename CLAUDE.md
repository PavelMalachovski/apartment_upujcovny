...
| `lightmaps` | Truthy = this apartment ships an offline lightmap pack under `tour/lightmaps/<id>/`, and the runtime should load it instead of baking those surfaces (`lightmaps.js`). Absent or falsy = bake at runtime, silently and with no fetch attempted. Only serenity sets it. **Not** in the hash input, so turning it on does not make its own pack stale. **Serenity's pack is a pilot that FAILED its own exit criterion and is still shipping while the decision is with the merge owner** — measured linear contrast 3.384 against a required ≥4.9, and a blind six-pair A/B that could not separate the frames at viewing size. It is not a no-op (it fills the ceiling/wall perimeter and the floor beside obstructions, up to ~100/255 locally) and it moves all-spot legacy ΔE the wrong way, 16.61→16.71. Do not extend it to another apartment on the strength of it being here. Verdict and costs both ways: `docs/superpowers/metrics/serenity-b3-task6-verdict.json` |
