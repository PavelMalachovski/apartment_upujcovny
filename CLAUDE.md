...
`window.__issues`, and `?check=1` shows a badge. ~~Four~~ **Six** bug classes:
blocked opening, opening into the void, room unreachable on foot,
marker inside a solid, and — **added 2026-08-26, and both fired on their first
run** — **unknown furniture type** (a `furniture` entry naming no `F.*`
constructor: `buildFurniture` used to `continue` past it, so the object was
simply absent and nothing said so — the class that cost an hour in rule 3's
stale-`index.html` note) and **no occlusion height** (a colliding type absent
from `OCC_H`, which silently took the 0.8 m default whatever the object's real
height — it found `shelfTower` (2.45 m) and `tvWallUnit` (2.35 m) baking their
shadows at 0.8 m in kings-court). Both are collected by `builder.js` into
`Builder.configIssues` and folded into `issues` by `validate.js`, so they are
covered by the rule below. **The list must be empty before commit.**
...
**Two cameras, and since 2026-08-26 they disagree by more than noise.**
`?fov=legacy` renders a fixed **72° vertical** with aspect only — for a 16:9
photograph that is **104° horizontal** — while `meta.photoFovLong` (serenity:
85) is the lens that models the photograph. The legacy camera exists for
continuity with PR #27's thresholds and is what every recorded serenity figure
before this date was measured through; the per-photograph camera is the one
that measures resemblance. **A camera pose chosen so the render frames its
photograph is penalised by the legacy camera**: re-posing serenity's 6.webp so
the frame holds the window seat, the lamp and the bed — which the old pose
missed entirely — moved it 9.23 → 12.32 at the gate, because an 8×8 grid of
cell mean colours rewards a featureless field of the right average colour.
Same lesson as the rejected mirrored-bedroom variant, from the other
direction. **Run and report both columns** when poses or lenses are in play;
`docs/superpowers/harnesses/2026-08-26b/legacy-pose-check.jpg` is the frame
that settles it.
