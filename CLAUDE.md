...
# No apartment ships a pack — the pilot was reverted, see the `lightmaps`
# row below. This is kept working, not currently used by anything.
...
| `lightmaps.js` | Offline lightmap pack: the loader and its staleness guard. `Lightmaps.load()` runs first thing inside `Baker.run` and returns a pack only if `apartments/<id>.json` sets `lightmaps` AND the manifest's SHA-256 still matches a canonical hash of the config's **geometry keys only** (`Lightmaps.GEOMETRY_KEYS`; renaming a room or re-fitting `exposure` deliberately does not invalidate a pack). Anything else — no pack, a stale hash, an image that will not load, a plate whose size or world position no longer matches its manifest entry — warns, pushes into `window.__issues`, and falls through to the runtime bake per surface. `window.__lightmaps` reports `off`/`missing`/`stale`/`nohash`/`ok`/`partial` — unlike `__issues` it is **not** synchronous, read it after `await window.__bakeReady`. `__bakeMs` now includes the manifest fetch and the texture loads, because they happen inside `Baker.run`. Baked by `tools/bake_lightmaps.mjs`, which never computes the hash itself — it asks the page for it, so one implementation decides both what to stamp and what to accept. **Dormant: no apartment ships a pack** (the serenity pilot was reverted — see the `lightmaps` config-key row), so `load()` returns null on its first line before any I/O and every apartment reports `status: "off"` with zero `/lightmaps/` requests |
...
| Key | One-liner |
|---|---|
| `exposure` | `renderer.toneMappingExposure` override, fitted per-apartment against its own photographs where it has any (`app.js`); an apartment with none is still fitted, not left at the default — horkyone-10 has zero `compare` spots and ships 0.46, fitted on mean-scene-luminance proximity to the other two (within ±10) instead of resemblance (`docs/superpowers/metrics/README.md`, "horkyone-10: fitted, and it passes the ±10 luminance check"). The bare default 1.05 is only what an apartment with an absent or invalid key falls back to. Must be a finite number `> 0` — `app.js` warns and falls back to 1.05 for anything else (`null`, `0`, a string). **Compensates for the scene running about three times as hot at source** (1.05 / 0.329 ≈ 3.2): serenity's fitted value is 0.329, kings-court's 0.575. Anyone correcting `lightAt`'s constants in `bake.js` must re-fit or clear this per-apartment override, or the render goes about three times too dark — and that is not hypothetical: phase B3 task 2 changed `lightAt`'s indoor ambient, and its own before/after (`tools/luminance.py`, the poseVerified compare spots, `?fov=legacy` — `*-b3-task2-luminance.json`) measures the cost as **−0.0058 of linear mean luminance at serenity (0.2854→0.2796) and −0.0063 at kings-court (0.3423→0.3360)**. Task 4 then re-fitted all three exposures upward to put the mean back: 0.326→0.329, 0.56→0.575, 0.45→0.46. |
...
| `lightmaps` | Truthy = this apartment ships an offline lightmap pack under `tour/lightmaps/<id>/`, and the runtime should load it instead of baking those surfaces (`lightmaps.js`). Absent or falsy = bake at runtime, silently and with no fetch attempted. **No apartment sets it, and no pack ships.** **Not** in the hash input, so turning it on does not make its own pack stale. **Serenity had a pilot pack; it FAILED its exit criterion and was REVERTED by the human partner's decision** — measured linear contrast 3.384 against a required ≥4.9, and a blind six-pair A/B that could not separate the frames at viewing size. It failed *by construction*, not by tuning: contrast is mean÷p5, so reaching 4.9 needed p5 to **fall** 31%, and bounce light **raised** it 2.5% — and the same conclusion holds on every population in the record, including the friendlier spawn-pooled one (~3.415 vs 4.9). It was not a no-op (it filled the ceiling/wall perimeter and the floor beside obstructions, up to ~100/255 locally) and it moved all-spot legacy ΔE the wrong way, 16.59→16.71/16.75; reverting returned that to 16.59. **Do not extend it to another apartment, and do not re-adopt it on serenity without redoing the exposure and bloom re-fit.** The loader here and `tools/bake_lightmaps.mjs` are kept and working but drive nothing. Baker, loader, pack, guard and all measurements are in git history at **`6a607fa`** (`git checkout 6a607fa -- tour/lightmaps/serenity` restores the pack without a 551 s re-bake). Verdict and costs both ways: `docs/superpowers/metrics/serenity-b3-task6-verdict.json` |
